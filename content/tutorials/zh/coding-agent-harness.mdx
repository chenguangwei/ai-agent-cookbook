...
difficulty: "Beginner"
duration: ""
thumbnail: "https://pbs.twimg.com/media/HMBC2b-XMAEaplQ.jpg"
featured: false
date: 2026-06-30
...
![](https://pbs.twimg.com/media/HMBBVnkW0AE1dLl.jpg)
...
![](https://pbs.twimg.com/media/HMBBWOEW8AEHHSQ.jpg)
...
>  -> big prompt
>  -> model response
>  -> run whatever tool it asked for
>  -> paste result back into prompt
>  -> repeat
...
![](https://pbs.twimg.com/media/HMBBXJ7W0AEQ8an.jpg)
...
>         return action.text
>  
>     result = validate_authorize_execute_record(action)
>     projected_history = update_context(result)
...
![](https://pbs.twimg.com/media/HMBBXwLXgAA0-7-.jpg)
...
![](https://pbs.twimg.com/media/HMBBYXXW4AAtvWe.jpg)
...
>  -> transcript now contains the file content
>  
> outside the agent
> formatter / user / git checkout
> changes src/config.py
>  
> turn 2
> model uses the old transcript text
> patch_file(
>   path="src/config.py",
>   old="TIMEOUT = 30",
>   new="TIMEOUT = 60")

问题不是 read_file。问题是磁盘变了以后，agent 仍然相信旧 read。
...
> User: change the timeout
> Tool: here is src/config.py
> Assistant: I will patch line 12
> Tool: patch succeeded
...
> path: src/config.py
> read range: whole file or lines 1-80
> baseline hash: abc123...
> mtime / size: last known disk fingerprint
> content baseline: bounded text the model saw
> source: read_file, agent_change, external_change
> status: fresh, changed, stale, partial, truncated
...
成熟的 harness 会把 read_file 看成一份 contract。

The harness contract:

> read_file(path)
>  -> record baseline
>  -> attach bounded content to transcript
>  -> remember what the model saw
>  
> before edit(path)
>  -> compare baseline with disk
>  -> reject stale / partial / missing / changed
>  -> require fresh read
>     or inject current changed lines
>  
> after successful edit(path)
>  -> record post-edit baseline
>  -> mark the agent's own write as fresh

这样，file read 从一个方便的工具，变成了 safety mechanism。

Model 依然可以提出 edit proposal。Harness 负责判断这个 proposal 是否基于最新的 file truth。

![](https://pbs.twimg.com/media/HMBBbLTXQAAhQxu.jpg)
...
> read_file("server.py", start=100, end=140)
>  -> useful context
>  -> partial baseline
>  -> not enough for whole-file overwrite
...
当 write_file 或 patch_file 成功后，要把 post-edit content 记录成新的 known baseline。当下一轮 prompt 构建时，让 Agent 去 lazy refresh tracked files：
...
- fresh read_file 会清除 stale 或 external-change state
...
![](https://pbs.twimg.com/media/HMBBb0mXYAAjs7n.jpg)
...
![](https://pbs.twimg.com/media/HMBBcyoWwAE_R8-.jpg)
...
> tool = tools[model_json["name"]]
> result = tool(**model_json["args"])
...
> name: write_file
> args schema: path, content
> risk: workspace_write
> path policy: must stay inside workspace
> state requirement: existing file needs fresh baseline
> approval: required for writes
> approval summary: path + operation + size
> execution: atomic write or exact patch
> result budget: bounded diff / chars / lines
> state update: record post-edit baseline
> transcript record: paired call and result
...
![](https://pbs.twimg.com/media/HMBBpJSWsAA3faF.jpg)
...
> stdout: clipped
> stderr: clipped
> large files: offset + limit
> search: max matches
> diff: compact preview
> binary data: metadata, not raw bytes
...
![](https://pbs.twimg.com/media/HMBBqK2XIAAoar4.jpg)
...
![](https://pbs.twimg.com/media/HMBBrINWIAAmiaO.jpg)
...
> session:
>   /status /memory /session /reset /help
> tools:
>   /tools
> goal:
...
> raw user input
>  -> empty? ignore
>  -> known slash command? handle locally
>  -> unknown slash command? return command help
>  -> local query? answer locally
>  -> normal request? prepare message and call model
...
![](https://pbs.twimg.com/media/HMBBsFCXcAAteo_.jpg)
...
> /audit /doctor inspect:
>  
> session history roles
> tool call / result pairing
> file-state freshness
...
> A coding-agent harness needs:
>  
> typed input router
> known local command handlers
> unknown command help
> session-state commands
> diagnostic commands
> narrow model mutation rules
> tests proving local commands do not call the model
...
>  -> project it into model context
>  -> apply tool and workflow constraints
...
> task mentions DongXi Agent
>  -> harness loads AGENTS.md as workspace guidance
>  -> note-capture protocol becomes active
>  -> session notes and concept map stay current
...
>  -> find applicable AGENTS.md files
>  -> read scoped instructions
>  -> add them to stable context
>  -> rebuild prompt with current task
...
![](https://pbs.twimg.com/media/HMBCFhZWQAAbkjx.jpg)
...
> ---
> name: review-notes
> description: Review learning notes for missing evidence.
...
>  
> 1. Read the note-capture protocol.
> 2. Search the relevant notes.
> 3. Check evidence, decisions, and next steps.
> 4. Return missing pieces with file references.
...
> scan skill folders:
>   .dongxi/skills/*/SKILL.md
>   .agents/skills/*/SKILL.md
>  
> default prompt sees metadata:
>   name, description, path, allowed tools
>  
> when relevant:
>   load_skill(name="review-notes")
>   -> read SKILL.md body
>   -> continue with skill active
...
![](https://pbs.twimg.com/media/HMBCGeYWwAAh4Xd.jpg)
...
![](https://pbs.twimg.com/media/HMBCHMOWkAAsLWu.jpg)
...
![](https://pbs.twimg.com/media/HMBCIG9XAAA23Mo.jpg)
...
> take the previous prompt
>  -> append the next user request
>  -> append every assistant message
...
> Phase 1: receive app input
> Phase 2: project working context
> Phase 3: ask the model
> Phase 4: execute tools and collect evidence
> Phase 5: commit transcript + app state
...
>  -> bounded working set for the next call
>  
> app state
>  -> file baselines, goals, validation, tools, tasks
...
![](https://pbs.twimg.com/media/HMBCJDiWkAA72vA.jpg)
...
> tool result: 80k chars
>  -> persist full output
>  -> keep preview + content_ref
>  -> record replacement metadata
>  -> show compact evidence to model
...
> long idle gap
>  -> keep recent N compactable results
>  -> clear or preview older tool output
>  -> preserve current task and state blocks
...
> compact boundary:
>   replaced turns 12-29
>   user asked for config refactor
>   files touched:
>     src/config.py
>     tests/test_config.py
>   key result: validation passed
>   open issue: retry timeout still hard-coded
...
![](https://pbs.twimg.com/media/HMBCKCmXkAAEpyX.jpg)
...
> Phase 3: model call
> Phase 4: tools and evidence
> Phase 5: next-state commit
...
![](https://pbs.twimg.com/media/HMBCKsBXQAAkUUR.jpg)

Harness 系列文章之 7，关于  subagent。

> Subagent 是一次 tool call，为 coding agent会打开一个新的 work context。

![](https://pbs.twimg.com/media/HMBCLsSWkAAItJQ.jpg)
...
> user:
>   Use a verifier agent for webhook retries.
>  
> parent tool:
>   spawn_agent({
>     task_name: "verify_retries",
>     agent_type: "verifier",
>     fork_turns: "3",
>     message: "Audit billing/webhook.ts..."
>   })
>  
> tool result:
>   /root/verify_retries
...
Slash commands are user-to-harness; spawn_agent is model-to-harness.
...
>  -> skills + summaries
>  -> recent turn
>  -> tool results + file state
>  
> Subagent
>  -> child session
>  -> fresh / fork / partial context
>  -> own turns
>  -> projected result
...
>   tools, files, docs, memories, subagent metadata
>  
> Child work context:
>   task
>   role
...
>  
> full fork
>  -> inherits parent history
>  -> needs a directive
>  
> partial fork
>  -> last N turns or selected slice
>  -> needs a tight boundary
...
![](https://pbs.twimg.com/media/HMBCKjOWcAEdZrt.jpg)
